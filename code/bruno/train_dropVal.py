...
aux1 = train_df[(train_df['TradeDateKey'] >= 20170501) & (train_df['TradeDateKey'] < 20170501)] # train a partir dessa data 
aux2 = train_df[(train_df['TradeDateKey'] >= 20160501) & (train_df['TradeDateKey'] < 20160501)] # train a partir dessa data 
train_df = train_df[train_df['TradeDateKey'] >= 20180101] # train a partir dessa data 
train_df = pd.concat([aux1, aux2, train_df])
...
valid_df.drop('DateKey', axis=1, inplace=True)
valid_df.drop('CustomerInterest', axis=1, inplace=True)
valid_df['diff'] = 1
k = pd.merge(k, valid_df, on=['CustomerIdx', 'IsinIdx'], how='left')
k = k.fillna(0)
k = k[k['diff'] == 1]
k.drop('diff', axis=1, inplace=True)
k.to_csv(path+"trainDropVal.csv", index=False)
